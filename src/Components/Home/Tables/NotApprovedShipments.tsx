...
const NotApprovedShipmentsTable = () => {
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(10);
  const [formattedData, setFormattedData] = useState<Data[]>([]);
  const ApproveShipmentHandler = ApproveShipmentMutation();

  const { data, loading, error, refetch } = useQuery<{ getNotApprovedProduct: ShipmentTypes[] }>(GET_SHIPMENTS_QUERY);
  ...
  const handleApprove = async (productId: string) => {
    const response = await ApproveShipmentHandler(productId);
  };
...
export default NotApprovedShipmentsTable;
